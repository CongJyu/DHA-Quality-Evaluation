...
class DataLoaderX(DataLoader):
    ...
    def my_collate_fn(batch):
        ...
        image_sequence = torch.utils.data.dataloader.default_collate(
            image_sequence)
        score_sequence = torch.utils.data.dataloader.default_collate(
            score_sequence)
        path_sequence = torch.utils.data.dataloader.default_collate(
            path_sequence)
...
class RankImageFolder(Dataset):
    ...
    def __init__(self, root, args):
        ...
        with open(os.path.join(root, file_name)) as label_file:
            for line in label_file:
                self.label_lines.append([p.strip()
    ...
    def __getitem__(self, index: int) -> (typing.Tuple[np.ndarray, np.ndarray],
        ...
        if self.is_rank:
            ...
            image_pair0 = Image.open(label_line[0])
            image_pair1 = Image.open(label_line[1])
            image_pair2 = Image.open(label_line[2])
            image_pair00 = self.loader(image_pair0.convert('RGB'))
            image_pair11 = self.loader(image_pair1.convert('RGB'))
            image_pair22 = self.loader(image_pair2.convert('RGB'))
            # image_pair = [image_pair00, image_pair11, image_pair22]
            ...
            path_pair = label_line[0:1]
            image_pair = [self.loader(label_line[0])]
            score_pair = [np.array([float(num)
                                   for num in label_line[1:]], dtype=np.float32)]
        else:
            logging.error("No Data set Reader Implemented for your Criterion, "
                          "only (rank, regress, emd) allow!")
            raise NotImplementedError(
                'No Data set Reader Implemented for your Criterion')
        return image_pair00, image_pair11, image_pair22, path_pair
...
if __name__ == '__main__':
    ...
    for my_samples, my_scores, image_paths in data_loader:
        ...
        my_samples = my_samples.numpy().transpose(
            0, 2, 3, 1).astype(np.uint8)[..., ::-1]
